...
async function buildAll() {
  // Build lib declaration files so TypeScript can resolve workspace references
  const tscBin = path.resolve(artifactDir, "../../node_modules/.bin/tsc");
  const libApiZod = path.resolve(artifactDir, "../../lib/api-zod");
  const libDb = path.resolve(artifactDir, "../../lib/db");
  const libApiClientReact = path.resolve(artifactDir, "../../lib/api-client-react");
  execSync(
    `"${tscBin}" --build "${libApiZod}" "${libDb}" "${libApiClientReact}"`,
    { stdio: "inherit" }
  );
